...
void tree::expression(char prefix[])
{
    ...
    int len, i;
    len = strlen(prefix);
    for (i = len - 1; i >= 0; i--) //start scanning from right to left ie len-1
    {
        top = new node;
        top->left = NULL;//initiate left and right child as null
        top->right = NULL;
        if (isalpha(prefix[i]))
        {
            top->data = prefix[i];
            s.push(top);//if operand then push into the stack
        }
        else if (prefix[i] == '+' || prefix[i] == '*' || prefix[i] == '-' || prefix[i] == '/')
        {
            t2 = s.pop();
            t1 = s.pop();//pop 2 elements
            top->data = prefix[i];//root-left-rigt format
            top->left = t2;
            top->right = t1;
            s.push(top);//push it again into the stack
        }
    }
    top = s.pop(); //return stack[top]
}
