...
void dict :: create()
{
 ...
 do
 {
  ...
  temp->left = NULL;//pointers of child nodes as null
  temp->right = NULL;
...
void dict ::  insert(node * root,node *temp)//*root->address of root node,temp->data to be inserted
{
 if(strcmp (temp->k, root->k) < 0 )//value is less than root hence inserted in left sub-tree
 {
  if(root->left == NULL)
   root->left = temp;
  else
