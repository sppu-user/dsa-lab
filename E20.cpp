...
void enqueue(string data,int p)//Enqueue function to insert data and its priority in queue
{
    ...
    else 
    {
        ...
        { //if there there is some elemets in Queue
            for(i=f;i<=r;i++) //to shift the elements.
            {
                Q[i-f] = Q[i];
                Pr[i-f] = Pr[i];
                r = r-f;
                f = 0;
                for(i = r;i>f;i--) //to find the correct position to insert the new element based on its priority
                {
                    if(p>Pr[i]) //compares p with priorities of existing elements (Pr[i])
                    {
                        Q[i+1] = Q[i]; //elements are shifted to the right to make space for the new element
                        Pr[i+1] = Pr[i];
                    }
                    else break;

                    Q[i+1] = data; //data and p are inserted when the correct position is found
                    Pr[i+1] = p;
                    r++;//r is inc to add new elements
                }
            }
        }
    else //there is enough space in the queue and the new element can be inserted directly
    {
        for(i = r;i>=f;i--) //traverses the queue from the rear to the front
        {
            if(p>Pr[i]) 
            {
                Q[i+1] = Q[i];
...
int main() 
{
    ...
    do 
    {
        cout << "1 for Insert the Data in Queue" << endl << "2 for show the Data in Queue " << endl <<
